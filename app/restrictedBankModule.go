...
// Handle MsgSend.
func handleMsgSend(ctx sdk.Context, k bankkeeper.Keeper, msg banktypes.MsgSend, ak authkeeper.AccountKeeper, oldHandler sdk.Handler) (*sdk.Result, error) {
	fmt.Println("new restricted bank handler handleMsgSend getting used - updated")

	acc := ak.GetAccount(ctx, sdk.AccAddress(msg.ToAddress))
